...
impl<K: Data, V: Data, H: Hasher> HashTable<K, V, H> {
    pub fn with_capacity_and_hasher(capacity: usize, hasher: H) -> Self {
        ...
        Self {
            data: (0..capacity).map(|_| Entry::<K, V>::empty()).collect::<Vec<_>>().into_boxed_slice(),
            hasher
        }
    }
    ...
    pub fn insert(&self, key: K, value: V) -> Result<(), V> {
        ...
        loop {
            ...
            if k == K::sentinel().to_u64() {
                if entry.key.compare_exchange_weak(k, key.to_u64(), Ordering::SeqCst, Ordering::SeqCst).is_err() {
                    // Someone already inserted something here - try again
                    continue;
                }
                match entry.value.compare_exchange_weak(V::sentinel().to_u64(), value.to_u64(), Ordering::SeqCst, Ordering::SeqCst) {
                    Ok(_) => {
                        return Ok(())
                    },
                    Err(existing_value) => {
                        ...
                        // entry"
                        return Err(V::from_u64(existing_value));
                    }
                }
    ...
    pub fn delete(&self, key: K) -> Option<V> {
        ...
        loop {
            ...
            if k == K::sentinel().to_u64() {
                ...
                match entry.value.compare_exchange_weak(v, V::sentinel().to_u64(), Ordering::SeqCst, Ordering::SeqCst) {
                    Ok(_) => {
...
mod tests {
    ...
    fn test_insert_exists() {
        let table = HashTable::<X, X, TestHash>::with_capacity(8);
        assert_eq!(table.insert(X(1), X(10)), Ok(())); 
        assert_eq!(table.insert(X(1), X(12)), Err(X(10))); 
        assert_eq!(table.lookup(X(1)), Some(X(10)));
    }
